...
// Minimal magic-byte sniff so a mislabeled extension doesn't silently
// slip through as a different format than it actually is.
function sniffFormat(bytes: Uint8Array): 'tiff' | 'png' | 'jpeg' | 'unknown' {
  if (bytes[0] === 0x49 && bytes[1] === 0x49 && bytes[2] === 0x2a && bytes[3] === 0x00) return 'tiff'; // little-endian TIFF
  if (bytes[0] === 0x4d && bytes[1] === 0x4d && bytes[2] === 0x00 && bytes[3] === 0x2a) return 'tiff'; // big-endian TIFF
...
/** Reads a dropped FileSystemEntry tree (files + nested folders) into a flat File[]. */
    ...
    if (entry.isFile) {
      ...
      const readAll = () => {
        reader.readEntries(async (batch) => {
          ...
          readAll(); // readEntries must be called repeatedly until it returns empty
        }, reject);
      };
      readAll();
    } else {
