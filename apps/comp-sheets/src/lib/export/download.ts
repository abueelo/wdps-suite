...
export async function saveStreamedFile(
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      await produce((chunk) => {
        // Not awaited for backpressure — acceptable at this app's scale
        // (competition-sized batches), and any error surfaces via the
        // outer produce()/writable.close() rejection.
      ...
      // Any other failure (e.g. picker unsupported in this context) —
      // fall through to the Blob download fallback below.
    }
  }
