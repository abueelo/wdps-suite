...
// mailbox) — this one is the actual running state of a competition night:
// every image, its rating and held flag, the current scene, the slide
// config. Same-origin, so the control panel (index.html) and the
// full-screen display (display.html) both read/write the same record
// without a server, the same way shared-bus lets apps talk to each other.

import { openDB, type IDBPDatabase } from 'idb';
import type { LiveSession } from '../types.js';
import { emptySession } from '../types.js';

const DB_NAME = 'wdps-presenter';
const STORE = 'session';
const DB_VERSION = 1;
